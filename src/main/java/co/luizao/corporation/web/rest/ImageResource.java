...
/**
 * REST controller for managing {@link co.luizao.corporation.domain.Image}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class ImageResource {
     ...
     */
    @GetMapping("/images")
    public List<Image> getAllImages(@RequestParam(required = false) String filter) {
        if ("artist-is-null".equals(filter)) {
            log.debug("REST request to get all Images where artist is null");
            return StreamSupport
                .stream(imageRepository.findAll().spliterator(), false)
                .filter(image -> image.getArtist() == null)
                .collect(Collectors.toList());
        }
        if ("album-is-null".equals(filter)) {
            log.debug("REST request to get all Images where album is null");
            return StreamSupport
                .stream(imageRepository.findAll().spliterator(), false)
