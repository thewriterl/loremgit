...
export class AlbumService {
  ...
  protected convertDateFromClient(album: IAlbum): IAlbum {
    const copy: IAlbum = Object.assign({}, album, {
      releaseDate: album.releaseDate && album.releaseDate.isValid() ? album.releaseDate.format(DATE_FORMAT) : undefined,
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((album: IAlbum) => {
        album.releaseDate = album.releaseDate ? moment(album.releaseDate) : undefined;
      });
    }
    return res;
  }
